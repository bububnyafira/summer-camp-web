 ...
 */ const build = async ({ logger, cwd, tsconfig, ...options })=>{
    ...
    try {
        ...
        if (ctx.bundler === 'webpack') {
            const { build: buildWebpack } = await Promise.resolve().then(function () { return require('./build-B6f5SABm.js'); });
            await buildWebpack(ctx);
        } else if (ctx.bundler === 'vite') {
            const { build: buildVite } = await Promise.resolve().then(function () { return require('./build-C4qN5x5L.js'); });
            await buildVite(ctx);
        }
...
const develop = async ({ cwd, polling, logger, tsconfig, watchAdmin, ...options })=>{
     ...
     */ let bundleWatcher;
        ...
        if (watchAdmin) {
            ...
            await writeStaticClientFiles(ctx);
            if (ctx.bundler === 'webpack') {
                const { watch: watchWebpack } = await Promise.resolve().then(function () { return require('./watch-CNfkm7Fm.js'); });
                bundleWatcher = await watchWebpack(ctx);
            } else if (ctx.bundler === 'vite') {
                const { watch: watchVite } = await Promise.resolve().then(function () { return require('./watch-gkyBsmAJ.js'); });
...
//# sourceMappingURL=index-DW2NjgW9.js.map
